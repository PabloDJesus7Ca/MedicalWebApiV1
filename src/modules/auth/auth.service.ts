  ...
  static async CheckLoginUserFromService(user: CheckTypeLoginDto): Promise<LoginResponseDto> {
    ...
    if (!userExist) {
      await logAudit(
        ...
        "User",
        undefined,
        `Intento de login con email inexistente: ${user.email}`
      );
      logger.warn(
        { email: user.email, accion: "LOGIN_FAILED_NO_USER" },
        `Intento de login fallido: email inexistente (${user.email})`
      );
    ...
    if (!passwordUser) {
      await logAudit(
        ...
        "User",
        userExist.id,
        `Contraseña incorrecta ingresada para Dr(a). ${userExist.nombre}`
      );
      logger.warn(
        { doctor_id: userExist.id, email: userExist.email, accion: "LOGIN_FAILED_WRONG_PASS" },
        `Intento de login fallido: contraseña incorrecta para ${userExist.nombre}`
      );
      throw new UserResponse(
        "Credenciales inválidas. El correo electrónico o la contraseña son incorrectos."
      );
    ...
    logger.info(
      { doctor_id: userExist.id, doctor_nombre: userExist.nombre, accion: "LOGIN_SUCCESS" },
      `Dr(a). ${userExist.nombre} inició sesión con éxito.`
    );
