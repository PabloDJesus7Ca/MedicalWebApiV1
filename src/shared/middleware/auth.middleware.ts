...
export const middlewareAuth = (
  ...
  try {
    ...
    logger.warn({ accion: "AUTH_FAILED_INVALID_TOKEN", error }, "Petición rechazada: Token inválido o expirado");
...
export const checkRoleMiddleware = (...allowedRoles: Rol[]) => {
  return (request: AuthRequest, response: Response, next: NextFunction): void => {
    if (!request.user) {
      ...
      logger.warn({ accion: "AUTH_FAILED_NO_USER_ROLE" }, "Petición rechazada en checkRole: Usuario no autenticado");
    ...
    if (!allowedRoles.includes(request.user.rol as Rol)) {
      ...
      logger.warn({ usuario_id: request.user.id, usuario_nombre: request.user.nombre, rol_actual: request.user.rol, roles_requeridos: allowedRoles, accion: "AUTH_FAILED_FORBIDDEN" }, `Acceso denegado (403): ${request.user.nombre || request.user.id} intentó acceder a ruta protegida.`);
      response
        .status(403)
        .json({
